...
const LearningStyleAssessment = () => {
  ...
  useEffect(() => {
    ...
    if (!currentUser) {
      navigate('/login', { state: { message: 'Please login to take the assessment', redirectTo: '/learning-assessment' } });
    } else if (location.state?.newUser) {
      // Show welcome message for new users
      setWelcomeMessage('Welcome! Please complete this assessment to help us personalize your experience.');    } else if (location.state?.fromLogin) {
  ...
  const saveLearningProfile = async (profile) => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/profile/learning-style',
        { profile },
        {
          headers: {
